...
def analyze_block(block_range):
    ...
    try:
        events = list()
        events += get_events(w3, client_version, {"fromBlock": block_range[0], "toBlock": block_range[1], "topics": [UNISWAP_V2]},  provider, "optimism", session)
        events += get_events(w3, client_version, {"fromBlock": block_range[0], "toBlock": block_range[1], "topics": [UNISWAP_V3]},  provider, "optimism", session)
        events += get_events(w3, client_version, {"fromBlock": block_range[0], "toBlock": block_range[1], "topics": [BALANCER_V1]}, provider, "optimism", session)
        events += get_events(w3, client_version, {"fromBlock": block_range[0], "toBlock": block_range[1], "topics": [BALANCER_V2]}, provider, "optimism", session)
        events += get_events(w3, client_version, {"fromBlock": block_range[0], "toBlock": block_range[1], "topics": [CURVE_1]},     provider, "optimism", session)
        events += get_events(w3, client_version, {"fromBlock": block_range[0], "toBlock": block_range[1], "topics": [CURVE_2]},     provider, "optimism", session)
    ...
    for block_number in events_per_block:
        status = mongo_connection["optimism"]["mev_arbitrage_status"].find_one({"block_number": block_number})
...
def main():
    ...
    for block in range(block_range_start, block_range_end+1):
        ...
        if counter == BLOCK_RANGE or block == block_range_end:
            block_range.append(block)
            count = mongo_connection["optimism"]["mev_arbitrage_status"].count_documents({"block_number": {"$gte": block_range[0], "$lte" : block_range[1]}})
    ...
    """mongo_connection = pymongo.MongoClient("mongodb://"+MONGO_HOST+":"+str(MONGO_PORT), maxPoolSize=None)
    results = list(mongo_connection["optimism"]["mev_arbitrage_results_copy"].find({}, {"block_number": 1, "_id": 0}))
    status = list(mongo_connection["optimism"]["mev_arbitrage_results"].find({}, {"block_number": 1, "_id": 0}))
    print(len(results))
    block_ranges = list()
    b = set()
    a = set()
    ...
    for document in results:
        if not document["block_number"] in b and not document["block_number"] in a and int(document["block_number"]) > 105235062:
